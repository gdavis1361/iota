...
def mock_prometheus_data():
    return [{"values": [[1614556800, "10.5"], [1614556860, "11.2"], [1614556920, "9.8"]]}]
...
def mock_prom():
    with patch("prometheus_api_client.PrometheusConnect") as mock:
        ...
        prom.custom_query_range.return_value = [
            {"values": [[1614556800, "10.5"], [1614556860, "11.2"], [1614556920, "9.8"]]}
        ]
        yield prom
...
def test_query_metric_formatting(mock_prom):
    ...
    reporter._query_metric("test_query", start_time, end_time)
