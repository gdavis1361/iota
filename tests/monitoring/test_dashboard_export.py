...
def mock_dashboard_list():
    return [{"id": 1, "uid": "abc123", "title": "Test Dashboard", "url": "/d/abc123"}]
...
def mock_dashboard_data():
    return {
        "dashboard": {
            ...
            "version": 1,
            "panels": [{"id": 1, "title": "Test Panel", "type": "graph"}],
        },
        "meta": {"isFolder": False, "folderId": 0},
    }
...
@pytest.mark.parametrize(
    "error_type", [requests.ConnectionError, requests.Timeout, requests.RequestException]
)
...
def test_invalid_api_key(mock_requests, tmp_path):
    mock_requests.return_value.raise_for_status.side_effect = requests.HTTPError(
