...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("chaos_tests.log")],
)
...
class RedisClient:
    ...
    @retry(
        stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=2, max=4), reraise=True
    )
...
async def test_network_delay_failover(redis_client, chaos_orchestrator):
    ...
    try:
        ...
        assert (
            baseline_metrics.write_success_rate > 0.95
        ), "Baseline writes should have high success rate"
        ...
        # Validate metrics
        assert (
            chaos_metrics.write_success_rate > 0.8
        ), "Write success rate should remain reasonable under delay"
...
async def test_cpu_pressure_failover(redis_client, chaos_orchestrator):
    ...
    try:
        ...
        assert (
            chaos_metrics.write_success_rate > 0.8
        ), "Write success rate should remain reasonable under CPU pressure"
...
async def test_memory_pressure_failover(redis_client, chaos_orchestrator):
    ...
    try:
        ...
        assert (
            chaos_metrics.write_success_rate > 0.8
        ), "Write success rate should remain reasonable under memory pressure"
        assert chaos_metrics.detection_time < 5.0, "Operations should complete within timeout"
...
async def test_combined_pressure_failover(redis_client, chaos_orchestrator):
    ...
    try:
        ...
        # Validate metrics
        assert (
            chaos_metrics.write_success_rate > 0.7
        ), "Write success rate should remain acceptable under combined pressure"
        assert (
            chaos_metrics.detection_time < 8.0
        ), "Operations should complete within extended timeout"
