...
def test_error_categorization(sample_logs):
    ...
    validation_errors = [e for e in analyzer.errors 
                        if e.get("category") == "CONFIG_VALIDATION"]
    assert len(validation_errors) == 2
    
    # Verify error details
    assert any(e.get("extra", {}).get("field") == "SENTRY_DSN" 
              for e in validation_errors)
    assert any(e.get("extra", {}).get("field") == "API_KEY" 
              for e in validation_errors)
