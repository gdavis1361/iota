...
@pytest.fixture
def rate_limiter(test_settings, mock_redis):
    """Create rate limiter with mock Redis."""
    return RateLimiter(test_settings.rate_limit_config, mock_redis)
...
def test_rate_limit_check_endpoint_specific(rate_limiter, mock_redis):
    ...
    allowed, wait_time, headers = rate_limiter.check_rate_limit(
        "127.0.0.1", endpoint="/test/endpoint"
    )
...
def test_rate_limit_exceeded(rate_limiter, mock_redis):
    ...
    mock_redis.ttl.return_value = 30
    mock_redis.incr.return_value = 102
