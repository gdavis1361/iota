...
sentinel_replica_health = Gauge(
    "redis_replica_health", "Health status of Redis replicas", ["replica"]
)
sentinel_failover_count = Counter(
    "redis_sentinel_failover_total", "Total number of failovers triggered"
)
sentinel_response_time = Histogram(
    "redis_sentinel_response_seconds", "Response time for Sentinel operations"
)
...
class SentinelHealthCheck:
    ...
    async def check_sentinel_quorum(self, master_name: str = "mymaster") -> Tuple[bool, Dict]:
        ...
        for host in self.sentinel_hosts:
            try:
                ...
                self.logger.info(
                    f"Sentinel {host} is healthy. Master: {master_info[3]}:{master_info[5]}, Replicas: {len(replicas)}"
                )

            except Exception as e:
                sentinel_health.labels(node=host).set(0)
                results[host] = {"status": "unhealthy", "error": str(e)}
        ...
        if not quorum_healthy:
            self.logger.error(
                f"Sentinel quorum unhealthy. Only {healthy_sentinels}/{len(self.sentinel_hosts)} sentinels available"
            )
    ...
    async def verify_master_status(self, master_host: str, master_port: int) -> bool:
        ...
        try:
            ...
            if is_master:
                ...
                self.logger.info(
                    f"Master {master_host}:{master_port} is healthy. Response time: {response_time:.3f}s"
                )
            else:
    ...
    async def verify_replica_status(self, replica_host: str, replica_port: int) -> bool:
        ...
        try:
            ...
            sentinel_replica_health.labels(replica=f"{replica_host}:{replica_port}").set(
                1 if status else 0
            )

            if status:
                self.logger.info(
                    f"Replica {replica_host}:{replica_port} is healthy and replicating"
                )
            else:
                self.logger.error(
                    f"Replica {replica_host}:{replica_port} status check failed. Role: {role_info[0]}, State: {role_info[3]}"
                )
