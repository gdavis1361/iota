...
def init_engine():
    ...
    try:
        logger.info(
            f"Attempting to connect to database at: {settings.POSTGRES_SERVER}:{settings.POSTGRES_PORT}"
        )
        logger.debug(
            f"Database connection parameters: user={settings.POSTGRES_USER}, db={settings.POSTGRES_DB}"
        )
        ...
        if engine and hasattr(engine, "pool"):
            ...
            if not isinstance(pool, NullPool):
                ...
                if pool.size() > 0:  # Avoid division by zero
                    ...
                    record_metrics(
                        "connection_pool_utilization", utilization, {"pool_name": "main"}
                    )
