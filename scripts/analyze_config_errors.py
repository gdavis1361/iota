...
class ConfigLogAnalyzer:
                ...
                with open(log_file) as f:
                    ...
                    for line in f:
                        try:
                            ...
                            if entry.get("level") == "ERROR" and "CONFIG" in entry.get(
                                "category", ""
                            ):
                                if cutoff:
    ...
    def print_summary(self) -> None:
        ...
        for error in self.errors[-3:]:
            ...
            print(
                f"  {COLORS['BLUE']}Details:{COLORS['RESET']} {json.dumps(error.get('extra', {}), indent=2)}"
            )
...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            f"{COLORS['RED']}Usage: analyze_config_errors.py <log_file1> [log_file2 ...] [--days=N]{COLORS['RESET']}"
        )
        sys.exit(1)
    ...
    for arg in sys.argv[1:]:
        if arg.startswith("--days="):
            try:
                ...
                print(
                    f"{COLORS['RED']}Invalid days argument. Use --days=N where N is a number.{COLORS['RESET']}"
                )
                sys.exit(1)
        else:
